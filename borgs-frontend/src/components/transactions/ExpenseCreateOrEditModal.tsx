...
export const ExpenseCreateOrEditModal = observer((props : ExpenseEditCreateModalProps) => {
	...
	const handleOnSubmitForm = (event) => {
		...
		if(props.isEditingMode) {
			transactionsStore.updateTransaction(transactionState)
		} else if(props.transactionType !== CategoryTypes.TRANSFER) {
	...
	let handleOnCurrencyValueChange;
	if(props.transactionType === CategoryTypes.EXPENSE) {
		defaultValueField = props.preFilledTransaction?.value ? -props.preFilledTransaction?.value : null;
		handleOnCurrencyValueChange = (newValue) => {
			handleOnValueChange("value", -newValue)
		}
	} else if (props.isEditingMode && props.transactionType === CategoryTypes.TRANSFER &&
		props.preFilledTransaction?.transaction_id === props.preFilledTransaction?.from_transfer_transaction) {
		defaultValueField = props.preFilledTransaction?.value ? -props.preFilledTransaction?.value : null;
		handleOnCurrencyValueChange = (newValue) => {
			handleOnValueChange("value", -newValue)
		}
	} else {
		defaultValueField = props.preFilledTransaction?.value;
		handleOnCurrencyValueChange = (newValue) => {
			handleOnValueChange("value", newValue)
		}
	}
							...
							<LocalizationProvider dateAdapter={AdapterDayjs}>
								<Stack spacing={2}>
									<TextField
										...
										onChange={(event) => {
											handleOnValueChange("description", event.target.value)
										}}
									/>
									<CurrencyField
										label="Value"
										defaultValue={defaultValueField}
										onChange={handleOnCurrencyValueChange}
									/>
									<DatePickerField
										label="Transaction date"
										defaultValue={preFilledTransaction.timestampepochseconds}
										onChange={(newDate) => {
											handleOnValueChange("timestampepochseconds", newDate?.unix())
										}}
									/>
									<CategoryPicker
										options={transactionsStore.availableCategories.filter((category) => {
											return category.category_type === props.transactionType
										})}
										defaultValue={preFilledTransaction.category}
										onChange={((event, category) => {
											handleOnValueChange("category", category)
										})}
										inputName={"category"}
									/>
									...
									<TagsPicker
										...
										onChange={(newTags) => {
											handleOnValueChange("tags", newTags)
										}}
									/>
	...
	return (
		<>
			<AccountPicker
				...
				defaultValue={currentVirtualAccountsData.find(account => account.account_id === preFilledTransaction.from_virtual_account)}
				onChange={((event, account) => { handleOnValueChange("virtual_account", account.account_id) })}
				inputName="virtual-account-picker"
			/>
			<AccountPicker
				...
				defaultValue={currentPhysicalAccountsData.find(account => account.account_id === preFilledTransaction.from_physical_account)}
				onChange={((event, account) => { handleOnValueChange("physical_account", account.account_id) })}
				inputName="physical-account-picker"
			/>
			<AccountPicker
				...
				defaultValue={currentVirtualAccountsData.find(account => account.account_id === preFilledTransaction.to_virtual_account)}
			...
			<AccountPicker
				...
				defaultValue={currentPhysicalAccountsData.find(account => account.account_id === preFilledTransaction.to_physical_account)}
